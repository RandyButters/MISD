    ...
    if env_name in ['AntMaze', 'PointMaze-v0', 'AntMaze-v0']:
        ...
        if evaluate:
            ...
            if manual_goal is not None:
                ...
                return lambda: np.random.uniform((-2, -2), (10, 10))
    elif env_name == 'AntFourRooms-v2':
        if evaluate:
            # return lambda: np.array([14., 14.])
            return lambda: np.array([0., 12.])
        else:
            return lambda: np.random.uniform((-3, -3), (15, 15))
    elif env_name == 'AntTwoRooms-v2':
        if evaluate:
            return lambda: np.array([16., 8.])
        else:
            return lambda: np.random.uniform((-1, -1), (17, 9))
    else:
...
def get_reward_fn(env_name, goal_dim, step_style=False):
    if step_style:
        ...
        if env_name in ['AntMaze-v1', 'AntMazeL-v1', 'PointMaze-v1']:
            ...
            return lambda obs, goal: -(np.linalg.norm(obs[:goal_dim] - goal, axis=-1) > 5.0).astype(np.float32)
        elif env_name in ["Spiral-v0", "NineRooms-v0"]:
            return lambda obs, goal: (np.linalg.norm(obs[:goal_dim] - goal, axis=-1) < 2.0).astype(np.float32)
        elif env_name in ["AntMazeS-v1"]:
            return lambda obs, goal: (np.linalg.norm(obs[:goal_dim] - goal, axis=-1) < 2.5).astype(np.float32)
        else:
            assert False, 'Unknown env'
    else:
        if env_name in ['AntMaze', 'PointMaze', "AntMazeSmall", "AntMazeL", "PointMaze-v1", "AntMaze-v1", "AntMaze-v0",
                        "AntMazeL-v1", "PointMaze-v0", "AntMazeW-v2", "AntFourRooms-v2", "AntFourRooms-v3",
                        "NineRooms-v0", "AntTwoRooms-v2", "Spiral-v0", "AntMazeS-v1"]:
...
def get_success_fn(env_name, step_style=False):
    if step_style:
        if env_name in [ 'AntMazeL-v1', 'PointMaze-v1',
                         "PointMaze-v0", "AntMazeW-v2", "AntMaze-v0", 
                          "NineRooms-v0"]:
            return lambda reward: reward == 0
        elif env_name in ["Spiral-v0"]:
            return lambda reward: reward > 1e-6
        elif env_name in ['AntMaze-v1', "AntMazeS-v1"]:
            return lambda reward: reward > 1e-6
        else:
            assert False, 'Unknown env'
    else:
        if env_name in ['AntMaze', 'AntMazeSmall', "AntMazeL", "PointMaze-v0", "AntMaze-v0"]:
            ...
            return lambda reward: reward > -5.0
        elif env_name == 'AntFourRooms-v3':
            return lambda reward: reward > -2.5
        elif env_name == 'NineRooms-v0':
            return lambda reward: reward > -2.0
        elif env_name == 'AntTwoRooms-v2':
            return lambda reward: reward > -2.5
        elif env_name == 'Spiral-v0':
            return lambda reward: reward > -2.0
        elif env_name == "AntMazeS-v1":
            return lambda reward: reward > -2.5
        else:
...
class EnvWithGoal(object):
    ...
    def __init__(self, base_env, env_name, fix_goal=False, manual_goals=None, step_style=False,
        ...
        if env_name in ['AntMaze-v1', "PointMaze-v0", "AntMazeSparse", "AntMazeW-v2",
                        "PointMaze-v1", "AntMaze-v0", "AntFourRooms-v2", "AntFourRooms-v3", 
                        "Spiral-v0", "NineRooms-v0", "AntTwoRooms-v2", "AntMazeS-v1"]:
            self.goal_dim = 2
        else:
        ...
        self.early_stop = False if (env_name in ['AntMaze', 'AntMazeSmall', "AntMazeL", "PointMaze-v1",
                                                 "AntMaze-v1", "AntMazeL-v1", "AntMaze-v0", "AntMazeW-v2",
                                                 "AntFourRooms-v2","AntFourRooms-v3", "Spiral-v0", 
                                                 "NineRooms-v0", "AntTwoRooms-v2", "AntMazeS-v1"]) else True
    ...
    def reset(self):
        # self.viewer_setup()
        self.early_stop_flag = False
        if self.env_name in ["Spiral-v0", "NineRooms-v0"]:
            if self.evaluate:
                obs = self.base_env.reset(fixed=True)
                self.goal = self.base_env.sample_goal(fixed=True)
            else:
                obs = self.base_env.reset(fixed=False)
                self.goal = self.base_env.sample_goal(fixed=True)
        else:
            obs = self.base_env.reset()
