...
class InfoNCE(nn.Module):
    def __init__(self, args, device):
        super().__init__()
        self.device = device
        self.tau = args.i_temperature
        self.CELoss = nn.CrossEntropyLoss()
        
    def forward(self, anchor, pos_key, neg_key):
        
        log_pos = (anchor @ pos_key.transpose(-1, -2)).squeeze(1)
        log_neg = (anchor @ neg_key.transpose(-1, -2)).squeeze(1)

        logits = torch.cat([log_pos, log_neg], dim=-1)
        labels = torch.zeros(len(logits), dtype=torch.long, device=self.device)
        loss = self.CELoss(logits / self.tau, labels)
    
        return loss
    
def train_info_net(i_net, traj_buffer, optimizer, device, args):
    traj = traj_buffer.get_trajectory()
    traj = np.array(traj)
    traj_num = traj.shape[0]
    traj_len = traj.shape[1]

    loss_func = InfoNCE(args, device)
    loss_avg = 0
    for i in range(args.i_training_epochs):
        epi_idx = np.random.randint(0, traj_num, size=args.i_batch_size)
        step_idx = np.random.randint(0, traj_len - 1, size=args.i_batch_size)
        neg_idx = np.random.randint(0, traj_len, size=args.i_neg_key_num) 
        
        anchor = traj[epi_idx, step_idx]
        pos_key = traj[epi_idx, step_idx+1]
        neg_key = traj[epi_idx][:, neg_idx]
        
        # B * M * N 
        anchor_tensor = torch.FloatTensor(anchor).unsqueeze(1).to(device)
        pos_key_tensor = torch.FloatTensor(pos_key).unsqueeze(1).to(device)
        neg_tensor = torch.FloatTensor(neg_key).to(device)
    
        anchor_embed = i_net(anchor_tensor)
        pos_key_embed = i_net(pos_key_tensor)
        neg_tensor_embed = i_net(neg_tensor)
        
        loss = loss_func(anchor_embed, pos_key_embed, neg_tensor_embed)
        loss_avg += loss.item()
    ...
    return loss_avg / args.i_training_epochs