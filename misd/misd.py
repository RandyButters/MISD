...
from misd.models import ControllerActor, ControllerCritic, ManagerActor, ManagerCritic, RndPredictor
# from misd.utils import RunningMeanStd
...
class Manager(object):
    ...
    def actor_loss(self, state, achieved_goal, next_achieved_goal, goal, a_net, i_net, r_margin, selected_landmark=None, no_pseudo_landmark=False):
        actions = self.actor(state, goal)
        eval = -self.critic.Q1(state, goal, actions).mean()
        norm = torch.norm(actions)*self.action_norm_reg
        # HIRO
        if a_net is None and i_net is None:
        ...
        # HRAC
        if a_net is not None and i_net is None:
            goal_loss = torch.clamp(F.pairwise_distance(a_net(achieved_goal), a_net(gen_subgoal)) - r_margin, min=0.).mean()
        
        # MISD
        if a_net is None and i_net is not None:
            goal_loss = -(1.0 - self.bound_coeff) * F.cosine_similarity(i_net(next_achieved_goal), i_net(gen_subgoal), dim=-1).mean() \
                    - self.bound_coeff * F.cosine_similarity(i_net(goal), i_net(gen_subgoal), dim=-1).mean()
                    
        if selected_landmark is None:
            return eval + norm, goal_loss, None, scaled_norm_direction  # HRAC, MISD
    ...
    def train(self,
        ...
        for it in range(iterations):
            ...
            next_achieved_goal = get_tensor(ag_next)
            goal = get_tensor(g)
            ...
            nn.utils.clip_grad_norm_(parameters=self.critic.parameters(), max_norm=10.0, norm_type=2)
            self.critic_optimizer.step()

            if algo == "hiro":
                actor_loss = self.actor_loss(state, achieved_goal, next_achieved_goal, goal, a_net, i_net, r_margin)

            elif algo == "misd" or algo == "test":
                assert i_net is not None
                actor_loss, goal_loss, _, _ = \
                    self.actor_loss(state, achieved_goal, next_achieved_goal, goal, a_net, i_net, r_margin)
                ...
                actor_loss, goal_loss, _, _ = \
                    self.actor_loss(state, achieved_goal, next_achieved_goal, goal, a_net, i_net, r_margin, selected_landmark=None)
                actor_loss = actor_loss + self.goal_loss_coeff * goal_loss
                avg_goal_loss += goal_loss
                ...
                actor_loss, goal_loss, ld_loss, scaled_norm_direction = self.actor_loss(state, achieved_goal, next_achieved_goal, goal,
                                                                                        a_net, i_net, r_margin,
                                                                                        selected_landmark,
            ...
            actor_loss.backward()
            nn.utils.clip_grad_norm_(parameters=self.actor.parameters(), max_norm=10.0, norm_type=2)
