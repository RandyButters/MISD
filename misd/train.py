...
def run_misd(args):
    ...
    elif "Spiral" in args.env_name or "NineRooms" in args.env_name:
        assert not args.stochastic_xy
        step_style = args.reward_shaping == 'sparse'
        env = EnvWithGoal(gym.make(args.env_name), env_name=args.env_name, step_style=step_style)
    else:
    ...
    if args.env_name in ["Reacher3D-v0"]:
        ...
        low = - high
    elif "AntMaze" in args.env_name or "PointMaze" in args.env_name or 'AntFourRooms' in args.env_name or 'AntTwoRooms' in args.env_name:
        high = np.array((10., 10.))
        low = - high
    elif "Spiral" in args.env_name or "NineRooms" in args.env_name:
        high = np.array((10., 10.))
    ...
    if args.absolute_goal:
        ...
        if "Point" in args.env_name or "Ant" in args.env_name or 'Spiral' in args.env_name:
            no_xy = True
        else:
            no_xy = False
    ...
    elif "Point" in args.env_name or "Ant" in args.env_name or "Spiral" in args.env_name or "NineRooms" in args.env_name:
    ...
    if args.algo in ['higl', 'hrac']:
        ...
        i_net = None
    elif args.algo in ['misd', 'test']:
        i_net = INet(controller_goal_dim, args.i_hidden_dim, args.i_embedding_dim)
        if args.load_info_net:
            print("Loading mutual information network...")
            i_net.load_state_dict(torch.load("{}/{}_{}_{}_{}_i_network.pth".format(args.load_dir,
                                                                                   ...
                                                                                   args.seed)))
        i_net.to(device)
        optimizer_i = optim.Adam(i_net.parameters(), lr=args.lr_i)
        a_net = None
    else:
        a_net = None
        i_net = None
    ...
    if args.algo == 'higl' and args.use_novelty_landmark:
        if args.novelty_algo == 'rnd':
            ...
            RND = misd.RandomNetworkDistillation(rnd_input_dim, args.rnd_output_dim, args.rnd_lr, args.use_ag_as_input)
        ...
        if done:
                ...
                if args.algo == 'higl' and args.use_novelty_landmark:
                    if args.novelty_algo == 'rnd':
                        if args.use_ag_as_input:
                            novelty = RND.get_novelty(np.array(ep_ac_seq).copy()).cpu()
                        else:
                            novelty = RND.get_novelty(np.array(ep_obs_seq).copy()).cpu()
            ...
            if total_timesteps != 0 and not just_loaded:
                ...
                if timesteps_since_eval >= args.eval_freq:
                    ...
                    if "Maze" in args.env_name or args.env_name in ["Reacher3D-v0", "Pusher-v0", "AntFourRooms-v2", "AntFourRooms-v3", 
                                                                    "Spiral-v0", "NineRooms-v0", "AntTwoRooms-v2"]:
                        ...
                        end_time = time.clock()
                        writer.add_scalar("eval/pers_env_goal_achieved", avg_env_finish, int(end_time-start_time))
                ...
                # Train mutual information network
                if args.algo in ["misd", "test"]:
                    if traj_buffer.full():
                        if not args.load_info_net:
                            print("Training mutual information network...")
                            utils.train_info_net(i_net, traj_buffer, optimizer_i, device, args)

                            if args.save_models:
                                r_filename = os.path.join(args.save_dir,
                                                          "{}_{}_{}_{}_i_network.pth".format(args.env_name,
                                                                                             args.algo,
                                                                                             args.version,
                                                                                             args.seed))
                                torch.save(i_net.state_dict(), r_filename)
                                print("----- Mutual information network {} saved. -----".format(episode_num))
                        traj_buffer.reset()
